...
import decimal
import logging
from market_robot.settings import settings
from market_robot.auth.APIKeyAuth import generate_expires, generate_signature
from market_robot.utils.log import setup_custom_logger
from market_robot.utils.math import toNearest
...
class BitMEXWebsocket():
    ...
    def connect(self, endpoint="", symbol="XBTUSD", shouldAuth=True, pubSubscription=[], priSubsctiption=[]):
        ...
        if pubSubscription:
            subscriptions = [sub + ':' + symbol for sub in pubSubscription]
        else:
            subscriptions = [sub + ':' + symbol for sub in ["instrument"]]
        if self.shouldAuth:
            if priSubsctiption:
                subscriptions += [sub + ':' + symbol for sub in priSubsctiption]
            else:
                subscriptions += [sub + ':' + symbol for sub in ["order", "execution", "margin", "position"]]
